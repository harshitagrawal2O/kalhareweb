...
function DivisionCard({ division, index, showBorder }: { division: (typeof divisions)[0]; index: number; showBorder: boolean }) {
  ...
  return (
    <motion.div
      ...
      className="relative flex-1 overflow-hidden cursor-none group border-[6px] transition-colors duration-500"
      onMouseMove={onMove}
      onMouseLeave={onLeave}
      onHoverStart={() => setHovered(true)}
      onHoverEnd={() => setHovered(false)}
      style={{ perspective: 1000, borderColor: hovered ? "#D4AF37" : "rgba(212,175,55,0.3)" }}
    >
    ...
    >
      ...
      <div id="divisions" className="relative z-10 flex flex-col md:flex-row flex-1 min-h-0">
        ...
        <motion.div
          ...
          className="hidden md:block absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-1.5 bg-gold/50"
